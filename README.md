# World of Claudecraft — WRKR Scale serving demo

This repository packages the full open-source World of Claudecraft game as a
standalone WRKR workload demo: a real browser MMO with a Node/WebSocket backend,
Postgres persistence, Docker deployment, health checks, and a documented
25-player target on one WRKR Scale VM.

The point is not to rebuild the game. The point is to show that a WRKR VM can
build, run, and serve a substantial real app end-to-end without Vercel,
Supabase, Clerk/Auth0, Pusher/Ably, Railway/Render/Fly, or another managed app
backend stack. Runtime services live on the VM:

- static browser client: served by the game server / Caddy
- backend: Node.js HTTP + WebSocket server
- realtime: local `ws` WebSocket server
- database: local Postgres
- auth/session state: app-owned tables in Postgres
- serving: Docker Compose plus Caddy or WRKR ingress

WRKR Scale target defaults:

- `MAX_PLAYERS=25`
- 4 vCPU / 8 GiB RAM / 100 GiB disk
- one realm/world per VM
- `/api/status` reports online players, cap, uptime, tick average, and memory
- `npm run load:25` verifies the 25-client WebSocket path

See [DEPLOY_WRKR.md](DEPLOY_WRKR.md) for the VM handoff runbook.

This demo preserves the upstream MIT license and third-party asset credits.

---

# Original Project: World of Claudecraft — a WoW-Classic-style MMO
