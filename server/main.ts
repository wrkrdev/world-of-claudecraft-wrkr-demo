...
function readMaxPlayers(raw: string | undefined): number {
  const n = Number(raw ?? DEFAULT_MAX_PLAYERS);
  return Number.isFinite(n) && n >= 1 ? Math.floor(n) : DEFAULT_MAX_PLAYERS;
}

const MAX_PLAYERS = readMaxPlayers(process.env.MAX_PLAYERS);
...
async function handleApi(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
  ...
  try {
    ...
    if (req.method === 'GET' && url === '/api/project-stats') {
      ...
      const stats = game.adminStats();
      return json(res, 200, {
        accounts_created: accountsCount,
        players_online: stats.online,
    ...
    if (req.method === 'GET' && url === '/api/status') {
      ...
      return json(res, 200, {
        ...
        players_online: stats.online,
        max_players: MAX_PLAYERS,
        peak_online: stats.peakOnline,
        uptime_seconds: stats.uptimeSeconds,
        tick_ms_avg: stats.tickMsAvg,
        sim_entities: stats.simEntities,
        rss_bytes: stats.rssBytes,
        heap_used_bytes: stats.heapUsedBytes,
...
async function main(): Promise<void> {
  ...
  async function authenticateWebSocket(ws: WebSocket, raw: string): Promise<void> {
    ...
    if (game.clients.size >= MAX_PLAYERS) {
      ws.send(JSON.stringify({ t: 'error', error: `realm is full (${MAX_PLAYERS} players online)` }));
      ws.close();
      return;
    }
    const character = await getCharacter(accountId, characterId);
